...
//		debugger
		...
		else
		{
			...
			result.SetWarning('Permisions for the current user:<br>' + ParsePermissions(CKFinderResourcesInfo));
		}
	}
	TestRunner.FinishedTest( result ) ;
}
...
//		result.SetError('IP: <br>' + resource.GetError() , null) ;
		...
		var hostIp = resource.GetSingleNodeValue( 'data/local_addr' ) ;
		serverName = resource.GetSingleNodeValue( 'data/server_name' ) ;

		if (aspNetIp == '0.0.0.0')
		{
			var message = 'Asp.Net isn\'t able to resolve the IP for the host "' + resource.GetSingleNodeValue( 'data/server_name' ) + '".<br>' ;
...
function ShowHostsHint()
{
			...
			message += '<b>127.0.0.1 ' + serverName + '</b><br>' ;
...
function SendFileTest()
{
	...
	(new FCKXml()).PostData(url, "test.txt", data, VerifySendFileTest, document.getElementById('sendFileTestLog') ) ;
	data = null;
}
...
function SendFile()
{
	...
	(new FCKXml()).PostData(url, "test.txt", data, VerifySendFile, document.getElementById('sendFileLog') ) ;
	data = null;
}
...
function DeleteTestFile(resourceType, fileName)
{
	...
	(new FCKXml()).LoadUrl(url, VerifyDeleteTestFile ) ;
}
...
function VerifyCreateTestImage( resource )
{
	...
	else
	{
		...
		if (response !== 'OK')
		{
			result.SetError( "Failed to create the test image in the server.<br>" + response, "") ;
		}
	}
	TestRunner.FinishedTest( result ) ;
}
...
//			document.body.appendChild(img) ;
			...
			result.SetError( "The server didn't send back an image.<br>" + response, "") ;
	}
	TestRunner.FinishedTest( result ) ;
}
