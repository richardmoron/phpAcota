		...
		try{
			if($valid){
				...
				if($aFormValues['txt_consumo_id'] == '0' )
					...
					$objac_consumosDAO->updateac_consumosSimple($objac_consumosTO);
	...
	function valid($aFormValues,$objResponse){
		...
		$objResponse = clearvalid($objResponse);
			...
			//--
                        ...
                        $criterio = array("socio_id"=>$aFormValues['txt_socio_id'],"periodo_mes"=>$aFormValues['txt_periodo_mes'],"periodo_anio"=>$aFormValues['txt_periodo_anio']);
                        $consumo_total_lectura = $ac_consumosDAO->totalAnterior_ac_consumos($criterio);
                        if($aFormValues['txt_consumo_total_lectura'] < $consumo_total_lectura){
                            $objResponse->script("alert('El consumo introducido no puede ser menor al de la última lectura: ".$consumo_total_lectura."');");
                            $valid = false;
                        }
