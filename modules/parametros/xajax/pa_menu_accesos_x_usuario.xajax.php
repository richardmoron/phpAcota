<?php 
	...
	*/
	...
	include_once (dirname(dirname(__FILE__))."/dao/pa_menu_accesos_x_usuario.dao.php");
...
//		$objResponse = loadUserLogged($objResponse);
	...
	function loadfields($key){
		...
		$objpa_menu_accesos_x_usuarioDAO = new pa_menu_accesos_x_usuarioDAO();
		$objpa_menu_accesos_x_usuarioTO = $objpa_menu_accesos_x_usuarioDAO->selectByIdpa_menu_accesos_x_usuario($key);

		$objResponse->assign("txt_menu_acceso_x_usuario_id","value",$objpa_menu_accesos_x_usuarioTO->getMenu_acceso_x_usuario_id());
		$objResponse->assign("txt_usuario_id","value",$objpa_menu_accesos_x_usuarioTO->getUsuario_id());
		$objResponse->assign("txt_menu_id","value",$objpa_menu_accesos_x_usuarioTO->getMenu_id());
                $objResponse->assign("txt_usuario_nombres","value",$objpa_menu_accesos_x_usuarioTO->getUsuario());
		$objResponse->assign("txt_menu","value",$objpa_menu_accesos_x_usuarioTO->getMenu());
		$objResponse->assign("txt_observaciones","value",$objpa_menu_accesos_x_usuarioTO->getObservaciones());
	...
	function loadtable_default($page_number,$arraylist,$table_count,$criterio){
		...
		$strhtml .= '<th scope="col">Menu<img src="'.MY_URI.'/media/img/tablesorter/bg.gif" alt="ordenar" class="order-img"/></th>'."\n";
		...
		$strhtml .= '<th colspan="'.(count(pa_menu_accesos_x_usuarioTO::$FIELDS)+3).'" id="footer_right">'."\n";
		//-- IBRAC
		...
		//-- IBRAC
			while($iterator->valid()) {
				...
				$objpa_menu_accesos_x_usuarioTO = $iterator->current();
				$strhtml .= '<td>'.$objpa_menu_accesos_x_usuarioTO->getUsuario().'</td>'."\n";
				$strhtml .= '<td>'.$objpa_menu_accesos_x_usuarioTO->getMenu().'</td>'."\n";
				//-- IBRAC
				$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="xajax_viewfields('.$objpa_menu_accesos_x_usuarioTO->getMenu_acceso_x_usuario_id().');"><img src="'.MY_URI.'/media/img/actions/view-row.png" alt="ver" title="ver" style="border:0"/></a></td>'."\n";
				if(strlen(strpos($criterio["ibrac"],"A"))>0)
					$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="xajax_loadfields('.$objpa_menu_accesos_x_usuarioTO->getMenu_acceso_x_usuario_id().');"><img src="'.MY_URI.'/media/img/actions/edit-row.png" alt="editar" title="editar" style="border:0"/></a></td>'."\n";
				else
					$strhtml .= '<td>&nbsp;</td>';
				if(strlen(strpos($criterio["ibrac"],"B"))>0)
					$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="if(confirm(\'Esta Seguro?\')){xajax_remove('.$objpa_menu_accesos_x_usuarioTO->getMenu_acceso_x_usuario_id().',xajax.getFormValues(\'formulario\'));}"><img src="'.MY_URI.'/media/img/actions/delete-row.png" alt="eliminar" title="eliminar" style="border:0"/></a></td>'."\n";
				else
					$strhtml .= '<td>&nbsp;</td>';
				//-- IBRAC
				$strhtml .= '</tr>'."\n";
	...
	function save($aFormValues){
		...
		try{
			if($valid){
				$objpa_menu_accesos_x_usuarioDAO = new pa_menu_accesos_x_usuarioDAO();
				$objpa_menu_accesos_x_usuarioTO = new pa_menu_accesos_x_usuarioTO();
				$objpa_menu_accesos_x_usuarioTO->setMenu_acceso_x_usuario_id($aFormValues['txt_menu_acceso_x_usuario_id']);
				$objpa_menu_accesos_x_usuarioTO->setUsuario_id($aFormValues['txt_usuario_id']);
				$objpa_menu_accesos_x_usuarioTO->setMenu_id($aFormValues['txt_menu_id']);
				$objpa_menu_accesos_x_usuarioTO->setObservaciones($aFormValues['txt_observaciones']);

				if($aFormValues['txt_menu_acceso_x_usuario_id'] == '0' )
					$objpa_menu_accesos_x_usuarioDAO->insertpa_menu_accesos_x_usuario($objpa_menu_accesos_x_usuarioTO);
				else
					$objpa_menu_accesos_x_usuarioDAO->updatepa_menu_accesos_x_usuario($objpa_menu_accesos_x_usuarioTO);
	...
	function remove($key,$aFormValues){
		$objpa_menu_accesos_x_usuarioDAO = new pa_menu_accesos_x_usuarioDAO();
		$objpa_menu_accesos_x_usuarioTO = new pa_menu_accesos_x_usuarioTO();
		$objpa_menu_accesos_x_usuarioTO->setMenu_acceso_x_usuario_id($key);
		$objpa_menu_accesos_x_usuarioDAO->deletepa_menu_accesos_x_usuario($objpa_menu_accesos_x_usuarioTO);
		return loadtable($aFormValues,0);
	}
	...
	function clearvalid($objResponse){
		$objResponse->assign("valid_menu_acceso_x_usuario_id","innerHTML", "");
	...
	function readOnlyfiles($state, $objResponse = null, $title = null){
		...
		$objResponse->script("readonly('formulario','txt_menu_acceso_x_usuario_id',$state)");
		...
		$objResponse->script("readonly('formulario','txt_menu_id',$state)");
                ...
                $objResponse->script("readonly('formulario','txt_menu',$state)");
	...
	function searchfields($aFormValues,$page_number){
		...
		$arraylist = $objpa_menu_accesos_x_usuarioDAO->selectByCriteria_pa_menu_accesos_x_usuario($criterio,$page_number);
		$table_count = $objpa_menu_accesos_x_usuarioDAO->selectCountpa_menu_accesos_x_usuario($criterio);
