<?php 
	...
	*/
	...
	include_once (dirname(dirname(__FILE__))."/dao/pa_permisos_x_area.dao.php");
...
//		$objResponse = loadUserLogged($objResponse);
	...
	function loadfields($key){
		...
		$objpa_permisos_x_areaDAO = new pa_permisos_x_areaDAO();
		$objpa_permisos_x_areaTO = $objpa_permisos_x_areaDAO->selectByIdpa_permisos_x_area($key);
                $objResponse->assign("ctn_cbx_area","innerHTML",  loadCbx_Area("_new", false));
                
		$objResponse->assign("txt_permisos_x_area_id","value",$objpa_permisos_x_areaTO->getPermisos_x_area_id());
		$objResponse->assign("cbx_area_new","value",$objpa_permisos_x_areaTO->getArea_id());
		$objResponse->assign("txt_archivo","value",$objpa_permisos_x_areaTO->getArchivo());
		$objResponse->assign("txt_insertar","value",$objpa_permisos_x_areaTO->getInsertar());
		$objResponse->assign("txt_borrar","value",$objpa_permisos_x_areaTO->getBorrar());
		$objResponse->assign("txt_reporte","value",$objpa_permisos_x_areaTO->getReporte());
		$objResponse->assign("txt_actualizar","value",$objpa_permisos_x_areaTO->getActualizar());
		$objResponse->assign("txt_consultar","value",$objpa_permisos_x_areaTO->getConsultar());
		$objResponse->assign("txt_observaciones","value",$objpa_permisos_x_areaTO->getObservaciones());
	...
	function loadtable_default($page_number,$arraylist,$table_count,$criterio){
		...
		$strhtml .= '<th colspan="'.(count(pa_permisos_x_areaTO::$FIELDS)+3).'" id="footer_right">'."\n";
		//-- IBRAC
		...
		//-- IBRAC
			while($iterator->valid()) {
				...
				$objpa_permisos_x_areaTO = $iterator->current();
				$strhtml .= '<td>'.$objpa_permisos_x_areaTO->getArea().'</td>'."\n";
				$strhtml .= '<td>'.$objpa_permisos_x_areaTO->getArchivo().'</td>'."\n";
                                if($objpa_permisos_x_areaTO->getInsertar() == "S")
                                    $strhtml .= '<td>SI</td>'."\n";
                                else
                                    $strhtml .= '<td>NO</td>'."\n";
                                
                                if($objpa_permisos_x_areaTO->getBorrar() == "S")
                                    $strhtml .= '<td>SI</td>'."\n";
                                else
                                    $strhtml .= '<td>NO</td>'."\n";
                                
                                if($objpa_permisos_x_areaTO->getReporte() == "S")
                                    $strhtml .= '<td>SI</td>'."\n";
                                else
                                    $strhtml .= '<td>NO</td>'."\n";
                                
                                if($objpa_permisos_x_areaTO->getActualizar() == "S")
                                    $strhtml .= '<td>SI</td>'."\n";
                                else
                                    $strhtml .= '<td>NO</td>'."\n";
                                
                                if($objpa_permisos_x_areaTO->getConsultar() == "S")
                                    $strhtml .= '<td>SI</td>'."\n";
                                else
                                    $strhtml .= '<td>NO</td>'."\n";
				//-- IBRAC
				$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="xajax_viewfields('.$objpa_permisos_x_areaTO->getPermisos_x_area_id().');"><img src="'.MY_URI.'/media/img/actions/view-row.png" alt="ver" title="ver" style="border:0"/></a></td>'."\n";
				if(strlen(strpos($criterio["ibrac"],"A"))>0)
					$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="xajax_loadfields('.$objpa_permisos_x_areaTO->getPermisos_x_area_id().');"><img src="'.MY_URI.'/media/img/actions/edit-row.png" alt="editar" title="editar" style="border:0"/></a></td>'."\n";
				else
					$strhtml .= '<td>&nbsp;</td>';
				if(strlen(strpos($criterio["ibrac"],"B"))>0)
					$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="if(confirm(\'Esta Seguro?\')){xajax_remove('.$objpa_permisos_x_areaTO->getPermisos_x_area_id().',xajax.getFormValues(\'formulario\'));}"><img src="'.MY_URI.'/media/img/actions/delete-row.png" alt="eliminar" title="eliminar" style="border:0"/></a></td>'."\n";
				else
					$strhtml .= '<td>&nbsp;</td>';
				//-- IBRAC
				$strhtml .= '</tr>'."\n";
	...
	function save($aFormValues){
		...
		try{
			if($valid){
				$objpa_permisos_x_areaDAO = new pa_permisos_x_areaDAO();
				$objpa_permisos_x_areaTO = new pa_permisos_x_areaTO();
				$objpa_permisos_x_areaTO->setPermisos_x_area_id($aFormValues['txt_permisos_x_area_id']);
				$objpa_permisos_x_areaTO->setArea_id($aFormValues['cbx_area_new']);
				$objpa_permisos_x_areaTO->setArchivo($aFormValues['txt_archivo']);
				$objpa_permisos_x_areaTO->setInsertar($aFormValues['txt_insertar']);
				$objpa_permisos_x_areaTO->setBorrar($aFormValues['txt_borrar']);
				$objpa_permisos_x_areaTO->setReporte($aFormValues['txt_reporte']);
				$objpa_permisos_x_areaTO->setActualizar($aFormValues['txt_actualizar']);
				$objpa_permisos_x_areaTO->setConsultar($aFormValues['txt_consultar']);
				$objpa_permisos_x_areaTO->setObservaciones($aFormValues['txt_observaciones']);

				if($aFormValues['txt_permisos_x_area_id'] == '0' )
					$objpa_permisos_x_areaDAO->insertpa_permisos_x_area($objpa_permisos_x_areaTO);
				else
					$objpa_permisos_x_areaDAO->updatepa_permisos_x_area($objpa_permisos_x_areaTO);
	...
	function add(){
		$objResponse = new xajaxResponse();
		$objResponse = clearvalid($objResponse);
		$objResponse->assign("txt_permisos_x_area_id","value","0");
	...
	function remove($key,$aFormValues){
		$objpa_permisos_x_areaDAO = new pa_permisos_x_areaDAO();
		$objpa_permisos_x_areaTO = new pa_permisos_x_areaTO();
		$objpa_permisos_x_areaTO->setPermisos_x_area_id($key);
		$objpa_permisos_x_areaDAO->deletepa_permisos_x_area($objpa_permisos_x_areaTO);
		return loadtable($aFormValues,0);
	}
	...
	function clearvalid($objResponse){
		$objResponse->assign("valid_permisos_x_area_id","innerHTML", "");
	...
	function readOnlyfiles($state, $objResponse = null, $title = null){
		...
		$objResponse->script("readonly('formulario','txt_permisos_x_area_id',$state)");
	...
	function searchfields($aFormValues,$page_number){
		...
		$arraylist = $objpa_permisos_x_areaDAO->selectByCriteria_pa_permisos_x_area($criterio,$page_number);
		$table_count = $objpa_permisos_x_areaDAO->selectCountpa_permisos_x_area($criterio);
