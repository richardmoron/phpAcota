<?php 
	...
	*/
	...
	include_once (dirname(dirname(__FILE__))."/dao/pa_areas.dao.php");
...
//		$objResponse = loadUserLogged($objResponse);
	...
	function loadfields($key){
		...
		$objpa_areasDAO = new pa_areasDAO();
		$objpa_areasTO = $objpa_areasDAO->selectByIdpa_areas($key);

		$objResponse->assign("txt_area_id","value",$objpa_areasTO->getArea_id());
		$objResponse->assign("txt_nombre_area","value",$objpa_areasTO->getNombre_area());
	...
	function loadtable_default($page_number,$arraylist,$table_count,$criterio){
		...
		$strhtml .= '<th colspan="'.(count(pa_areasTO::$FIELDS)+3).'" id="footer_right">'."\n";
		//-- IBRAC
		...
		//-- IBRAC
			while($iterator->valid()) {
				...
				$objpa_areasTO = $iterator->current();
				$strhtml .= '<td>'.$objpa_areasTO->getNombre_area().'</td>'."\n";
				//-- IBRAC
				$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="xajax_viewfields('.$objpa_areasTO->getArea_id().');"><img src="'.MY_URI.'/media/img/actions/view-row.png" alt="ver" title="ver" style="border:0"/></a></td>'."\n";
				if(strlen(strpos($criterio["ibrac"],"A"))>0)
					$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="xajax_loadfields('.$objpa_areasTO->getArea_id().');"><img src="'.MY_URI.'/media/img/actions/edit-row.png" alt="editar" title="editar" style="border:0"/></a></td>'."\n";
				else
					$strhtml .= '<td>&nbsp;</td>';
				if(strlen(strpos($criterio["ibrac"],"B"))>0)
					$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="if(confirm(\'Esta Seguro?\')){xajax_remove('.$objpa_areasTO->getArea_id().',xajax.getFormValues(\'formulario\'));}"><img src="'.MY_URI.'/media/img/actions/delete-row.png" alt="eliminar" title="eliminar" style="border:0"/></a></td>'."\n";
				else
					$strhtml .= '<td>&nbsp;</td>';
				//-- IBRAC
				$strhtml .= '</tr>'."\n";
	...
	function save($aFormValues){
		...
		try{
			if($valid){
				$objpa_areasDAO = new pa_areasDAO();
				$objpa_areasTO = new pa_areasTO();
				$objpa_areasTO->setArea_id($aFormValues['txt_area_id']);
				$objpa_areasTO->setNombre_area(strtoupper($aFormValues['txt_nombre_area']));

				if($aFormValues['txt_area_id'] == '0' )
					$objpa_areasDAO->insertpa_areas($objpa_areasTO);
				else
					$objpa_areasDAO->updatepa_areas($objpa_areasTO);
	...
	function add(){
		$objResponse = new xajaxResponse();
		$objResponse = clearvalid($objResponse);
		$objResponse->assign("txt_area_id","value","0");
		$objResponse->assign("txt_nombre_area","value","");
	...
	function remove($key,$aFormValues){
		$objpa_areasDAO = new pa_areasDAO();
		$objpa_areasTO = new pa_areasTO();
		$objpa_areasTO->setArea_id($key);
		$objpa_areasDAO->deletepa_areas($objpa_areasTO);
		return loadtable($aFormValues,0);
	}
	...
	function valid($aFormValues,$objResponse){
		$valid = true;
		$objResponse = clearvalid($objResponse);
			if(trim($aFormValues['txt_area_id']) == ""){
				$objResponse->assign("valid_area_id","innerHTML", '<img src="../../../../media/img/actions/alert.png" alt="Requerido" title="Requerido" />');
				$valid = false;
			}
			if(trim($aFormValues['txt_nombre_area']) == ""){
				$objResponse->assign("valid_nombre_area","innerHTML", '<img src="../../../../media/img/actions/alert.png" alt="Requerido" title="Requerido" />');
				$valid = false;
			}
		return $valid;
	}
	function clearvalid($objResponse){
		...
		$objResponse->assign("valid_nombre_area","innerHTML", "");
		return $objResponse;
	}
	...
	function readOnlyfiles($state, $objResponse = null, $title = null){
		...
		$objResponse->script("readonly('formulario','txt_nombre_area',$state)");
		if($state == "false"){
			$objResponse = visibilityButton("savebutton", "visible", $objResponse);
                        $objResponse = visibilityButton("editbutton", "hidden", $objResponse);
                }
	...
	function searchfields($aFormValues,$page_number){
		...
		//-- IBRAC
                if(isset($aFormValues["txts_nombre"]))
			$criterio["nombre_area"] = $aFormValues["txts_nombre"];
                
		$arraylist = $objpa_areasDAO->selectByCriteria_pa_areas($criterio,$page_number);
		$table_count = $objpa_areasDAO->selectCountpa_areas($criterio);
