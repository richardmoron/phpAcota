<?php 
	...
	*/
	...
	include_once (dirname(dirname(__FILE__))."/dto/pa_permisos_x_usuario.dto.php");
	include_once (dirname(dirname(dirname(dirname(__FILE__))))."/lib/logs.php");

	class pa_permisos_x_usuarioDAO {
		private $connection;
 
		function pa_permisos_x_usuarioDAO(){
		}

		/**
		* Añade un Registro
		*
		* @param pa_permisos_x_usuarioTO $elem
		* @return int Filas Afectadas
		*/
		function insertpa_permisos_x_usuario(pa_permisos_x_usuarioTO $elem){
			$this->connection = Connection::getinstance()->getConn();
			$PreparedStatement = "INSERT INTO pa_permisos_x_usuario (permisos_x_usuario_id,usuario_id,archivo,insertar,borrar,reporte,actualizar,consultar,observaciones ) VALUES (
				".Connection::inject($elem->getPermisos_x_usuario_id()).",
		...
		* @return int Filas Afectadas
		*/
		function updatepa_permisos_x_usuario(pa_permisos_x_usuarioTO $elem){
			$this->connection = Connection::getinstance()->getConn();
			$PreparedStatement = "UPDATE pa_permisos_x_usuario SET  
			...
			WHERE permisos_x_usuario_id = ". Connection::inject($elem->getPermisos_x_usuario_id()).";" ;
		...
		* @return int Filas Afectadas
		*/
		function deletepa_permisos_x_usuario(pa_permisos_x_usuarioTO $elem){
			$this->connection = Connection::getinstance()->getConn();
			$PreparedStatement = "DELETE FROM pa_permisos_x_usuario WHERE permisos_x_usuario_id = ". Connection::inject($elem->getPermisos_x_usuario_id()).";";
		...
		/**
		* Obtiene un objeto pa_permisos_x_usuarioTO
		*
		* @return pa_permisos_x_usuarioTO elem
		*/
		function selectByIdpa_permisos_x_usuario($permisos_x_usuario_id){
			$this->connection = Connection::getinstance()->getConn();
			$PreparedStatement = "SELECT permisos_x_usuario_id, usuario_id, archivo, insertar, borrar, reporte, actualizar, consultar, observaciones,
                                                (SELECT CONCAT(usuario,', ',apellidos,' ',nombres) AS usuario FROM pa_usuarios WHERE usuario_id = pa_permisos_x_usuario.usuario_id) AS usuario 
                                                FROM pa_permisos_x_usuario WHERE permisos_x_usuario_id = ".Connection::inject($permisos_x_usuario_id)." ;";
			$ResultSet = mysql_query($PreparedStatement,$this->connection);
			logs::set_log(__FILE__,__CLASS__,__METHOD__, $PreparedStatement);

			$elem = new pa_permisos_x_usuarioTO();
			while($row = mysql_fetch_array($ResultSet)){
				$elem = new pa_permisos_x_usuarioTO();
				$elem->setPermisos_x_usuario_id($row['permisos_x_usuario_id']);
		...
		* @return int $rows
		*/
		function selectCountpa_permisos_x_usuario($criterio){
			$this->connection = Connection::getinstance()->getConn();
			$PreparedStatement = "SELECT COUNT(*) AS count FROM pa_permisos_x_usuario WHERE permisos_x_usuario_id = permisos_x_usuario_id ";
		...
		*
		* @return ArrayObject pa_permisos_x_usuarioTO
		* @param array $criterio
		*/
		function selectByCriteria_pa_permisos_x_usuario($criterio,$page_number){
			$this->connection = Connection::getinstance()->getConn();
			$PreparedStatement = "SELECT permisos_x_usuario_id, usuario_id, archivo, insertar, borrar, reporte, actualizar, consultar, observaciones,
                                                 (SELECT CONCAT(usuario,', ',apellidos,' ',nombres) AS usuario FROM pa_usuarios WHERE usuario_id = pa_permisos_x_usuario.usuario_id) AS usuario    
						 FROM pa_permisos_x_usuario WHERE permisos_x_usuario_id = permisos_x_usuario_id ";

			$PreparedStatement = $this->defineCriterias($criterio,$PreparedStatement);
			$PreparedStatement .= " ORDER BY permisos_x_usuario_id LIMIT ".($page_number*TABLE_ROW_VIEW).",". ($page_number+1)*TABLE_ROW_VIEW."";
			$ResultSet = mysql_query($PreparedStatement,$this->connection);
			logs::set_log(__FILE__,__CLASS__,__METHOD__, $PreparedStatement);
		...
		*
		* @return String $PreparedStatement pa_permisos_x_usuarioTO
		* @param String $PreparedStatement
		* @param array $criterio
		*/
		function defineCriterias($criterio,$PreparedStatement){
			if(isset ($criterio["permisos_x_usuario_id"]) && trim($criterio["permisos_x_usuario_id"]) != "0"){
				$PreparedStatement .=" AND permisos_x_usuario_id = ".Connection::inject($criterio["permisos_x_usuario_id"]);
			}
