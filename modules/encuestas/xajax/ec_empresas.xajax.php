<?php 
	...
	*/
	...
	include_once (dirname(dirname(__FILE__))."/dao/ec_empresas.dao.php");
...
//		$objResponse = loadUserLogged($objResponse);
	...
	function loadfields($key){
		...
		$objec_empresasDAO = new ec_empresasDAO();
		$objec_empresasTO = $objec_empresasDAO->selectByIdec_empresas($key);

		$objResponse->assign("txt_empresa_id","value",$objec_empresasTO->getEmpresa_id());
		$objResponse->assign("txt_nombre","value",$objec_empresasTO->getNombre());
		$objResponse->assign("txt_direccion","value",$objec_empresasTO->getDireccion());
		$objResponse->assign("txt_telefono","value",$objec_empresasTO->getTelefono());
		$objResponse->assign("txt_persona_contacto","value",$objec_empresasTO->getPersona_contacto());
	...
	function loadtable_default($page_number,$arraylist,$table_count,$criterio){
		...
		$strhtml .= '<th scope="col">Telefono<img src="'.MY_URI.'/media/img/tablesorter/bg.gif" alt="ordenar" class="order-img"/></th>'."\n";
		$strhtml .= '<th scope="col">Persona Contacto<img src="'.MY_URI.'/media/img/tablesorter/bg.gif" alt="ordenar" class="order-img"/></th>'."\n";
		...
		$strhtml .= '<th colspan="'.(count(ec_empresasTO::$FIELDS)+3).'" id="footer_right">'."\n";
		//-- IBRAC
		...
		//-- IBRAC
			while($iterator->valid()) {
				...
				$objec_empresasTO = $iterator->current();
				$strhtml .= '<td>'.$objec_empresasTO->getNombre().'</td>'."\n";
				$strhtml .= '<td>'.$objec_empresasTO->getTelefono().'</td>'."\n";
				$strhtml .= '<td>'.$objec_empresasTO->getPersona_contacto().'</td>'."\n";
				//-- IBRAC
				$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="xajax_viewfields('.$objec_empresasTO->getEmpresa_id().');"><img src="'.MY_URI.'/media/img/actions/view-row.png" alt="ver" title="ver" style="border:0"/></a></td>'."\n";
				if(strlen(strpos($criterio["ibrac"],"A"))>0)
					$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="xajax_loadfields('.$objec_empresasTO->getEmpresa_id().');"><img src="'.MY_URI.'/media/img/actions/edit-row.png" alt="editar" title="editar" style="border:0"/></a></td>'."\n";
				else
					$strhtml .= '<td>&nbsp;</td>';
				if(strlen(strpos($criterio["ibrac"],"B"))>0)
					$strhtml .= '<td style="width:5px;"><a href="#" target="_self" onclick="if(confirm(\'Esta Seguro?\')){xajax_remove('.$objec_empresasTO->getEmpresa_id().',xajax.getFormValues(\'formulario\'));}"><img src="'.MY_URI.'/media/img/actions/delete-row.png" alt="eliminar" title="eliminar" style="border:0"/></a></td>'."\n";
				else
					$strhtml .= '<td>&nbsp;</td>';
				//-- IBRAC
				$strhtml .= '</tr>'."\n";
	...
	function save($aFormValues){
		...
		try{
			if($valid){
				$objec_empresasDAO = new ec_empresasDAO();
				$objec_empresasTO = new ec_empresasTO();
				$objec_empresasTO->setEmpresa_id($aFormValues['txt_empresa_id']);
				$objec_empresasTO->setNombre($aFormValues['txt_nombre']);
				$objec_empresasTO->setDireccion($aFormValues['txt_direccion']);
				$objec_empresasTO->setTelefono($aFormValues['txt_telefono']);
				$objec_empresasTO->setPersona_contacto($aFormValues['txt_persona_contacto']);

				if($aFormValues['txt_empresa_id'] == '0' )
					$objec_empresasDAO->insertec_empresas($objec_empresasTO);
				else
					$objec_empresasDAO->updateec_empresas($objec_empresasTO);
	...
	function add(){
		$objResponse = new xajaxResponse();
		$objResponse = clearvalid($objResponse);
		$objResponse->assign("txt_empresa_id","value","0");
		$objResponse->assign("txt_nombre","value","");
		$objResponse->assign("txt_direccion","value","");
		$objResponse->assign("txt_telefono","value","");
		$objResponse->assign("txt_persona_contacto","value","");
	...
	function remove($key,$aFormValues){
		$objec_empresasDAO = new ec_empresasDAO();
		$objec_empresasTO = new ec_empresasTO();
		$objec_empresasTO->setEmpresa_id($key);
		$objec_empresasDAO->deleteec_empresas($objec_empresasTO);
		return loadtable($aFormValues,0);
	}
	...
	function valid($aFormValues,$objResponse){
		$valid = true;
		$objResponse = clearvalid($objResponse);
			if(trim($aFormValues['txt_empresa_id']) == ""){
			...
			if(trim($aFormValues['txt_telefono']) == ""){
				$objResponse->assign("valid_telefono","innerHTML", '<img src="../../../../media/img/actions/alert.png" alt="Requerido" title="Requerido" />');
				$valid = false;
			}
			if(trim($aFormValues['txt_persona_contacto']) == ""){
				$objResponse->assign("valid_persona_contacto","innerHTML", '<img src="../../../../media/img/actions/alert.png" alt="Requerido" title="Requerido" />');
				$valid = false;
			}
		return $valid;
	}
	function clearvalid($objResponse){
		...
		$objResponse->assign("valid_telefono","innerHTML", "");
		$objResponse->assign("valid_persona_contacto","innerHTML", "");
		return $objResponse;
	}
	...
	function readOnlyfiles($state, $objResponse = null, $title = null){
		...
		$objResponse->script("readonly('formulario','txt_telefono',$state)");
		$objResponse->script("readonly('formulario','txt_persona_contacto',$state)");
		if($state == "false"){
			$objResponse = visibilityButton("savebutton", "visible", $objResponse);
			$objResponse = visibilityButton("editbutton", "hidden", $objResponse);
		}else{
	...
	function searchfields($aFormValues,$page_number){
		...
		//-- IBRAC
		$arraylist = $objec_empresasDAO->selectByCriteria_ec_empresas($criterio,$page_number);
		$table_count = $objec_empresasDAO->selectCountec_empresas($criterio);
